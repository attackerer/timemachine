...
class ProcessThread(QThread):
    ...
    def run(self):
        print('run runs')
        def Def_go_back_func():
            import sys
            from _datetime import datetime

            time_tuple = (2000,  # Year
                          9,  # Month
                          6,  # Day
                          0,  # Hour
                          38,  # Minute
            ...
            if __name__ == "__main__":
                ...
                for server in server_list:
                    ...
                    if epoch_time is not None:
                        ...
                        win32api.SetSystemTime(utcTime.year, utcTime.month, utcTime.weekday(), utcTime.day,
                                               utcTime.hour, utcTime.minute, utcTime.second, 0)
        ...
        if self.UI.pb_go_back_status == 1:
            Def_go_back_func()
        if self.UI.pb_sync_time_status == 1:
            Def_sync_time_func()


class Ui_MainWindow(object):
    pb_go_back_status=0
    pb_sync_time_status = 0
...
"}")
        ...
        self.pb_go_back.clicked.connect(self.go_back)
        self.pb_sync_time.clicked.connect(self.sync_time)
        QtCore.QMetaObject.connectSlotsByName(MainWindow)

    def go_back(self):
        print('pb go back clicked')
        pb_go_back_status = 1
        print('pb go back status = 1')
        self.PT = ProcessThread(self)
        self.PT.start()
        print('start threath')
    def sync_time(self):
        pb_sync_time_status = 1
        self.PT = ProcessThread(self)
        self.PT.start()
